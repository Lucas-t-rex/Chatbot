...
def receive_webhook():
    try:
        ...
        if clean_number != CONFIG["RESPONSIBLE_NUMBER"] and clean_number not in CLIENTES_EM_INTERVENCAO:
            ...
            msg_aviso = f"🔔 *INTERVENÇÃO HUMANA*\nO cliente {clean_number} respondeu.\nBot pausado."
...
# ==============================================================================
# 🧵 INICIALIZAÇÃO DA THREAD (AGORA FORA DO MAIN)
# ==============================================================================
def loop_disparo():
    # Delay inicial para dar tempo do servidor web subir no Fly
    ...
    if df.empty:
        print("⚠️ Nenhuma lista encontrada ou lista vazia.")
        return
...
# Iniciamos a thread assim que o arquivo é importado pelo Gunicorn
# Usamos um verificador simples para garantir que rode apenas uma vez (caso Gunicorn recarregue)
if not os.environ.get("WERKZEUG_RUN_MAIN"): # Evita duplicidade em reloaders locais
