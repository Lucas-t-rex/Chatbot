...
def gerar_msg_pos_agendamento_ia(nome_cliente, status):
    ...
    if status == 'concluido':
        ...
        prompt = f"""
        O cliente ({nome_fala}) estava agendado para vir na Brooklyn Academia ontem, mas não pôde comparecer.
        SUA MISSÃO: Mandar uma mensagem curta e acolhedora que convide o cliente a reagendar.

        1. Diga de forma leve que sentiu falta dele ontem.
        2. Pergunte diretamente quando ele consegue vir, oferecendo reagendar agora mesmo.

        Regras:
        - Tom animado e receptivo, nunca cobrança.
        - Foco em trazer o cliente de volta, não em saber o que aconteceu.
        ...
        for ag in pendentes:
            ...
            if not msg:
                if status_ag == 'concluido':
                    ...
                    msg = f"{nome_cliente}, sentimos sua falta ontem! Quando você consegue vir? Posso já te reagendar aqui 😊"
...
def extrair_nome_da_msg_admin(texto_msg):
    """
    Detecta nome próprio de pessoa endereçado ao cliente em mensagem manual do admin.
    Ex: "Oi Marcos, você está devendo" → "Marcos"
        "Tudo bem?" → None
        "Bom dia querida" → None (apelido genérico)
        "Isaque Ferramentas" → None (empresa)
    """
    if not modelo_ia or not texto_msg or len(texto_msg.strip()) < 3:
        return None
    try:
        prompt = f"""Você analisa uma mensagem que um atendente humano enviou para um cliente via WhatsApp.
Sua missão: identificar se a mensagem CHAMA o cliente por um NOME PRÓPRIO de pessoa.

MENSAGEM: "{texto_msg}"

REGRAS:
- Só extraia se for claramente um nome próprio de pessoa (Marcos, Ana, Isaque, João, Maria, Pedro, etc).
- IGNORE apelidos genéricos: amor, querida, fofo, mano, cara, irmão, chefe.
- IGNORE marcas/nomes de empresa: "Isaque Ferramentas" → NAO (é empresa, não pessoa).
- IGNORE frases religiosas/saudações: "Deus é fiel", "Bom dia", "Olá".
- Se houver dúvida, responda NAO.

Responda APENAS o primeiro nome (uma palavra) ou exatamente: NAO"""
        resp = modelo_ia.generate_content(prompt)
        result = (resp.text or "").strip()
        if not result or result.upper() in ("NAO", "NÃO", "NONE", "N/A"):
            return None
        primeira = result.split()[0].strip(".,!?;:\"'").strip()
        if len(primeira) < 2 or len(primeira) > 25 or not primeira[0].isalpha():
            return None
        return primeira.title()
    except Exception as e:
        print(f"⚠️ Erro em extrair_nome_da_msg_admin: {e}")
        return None


def try_auto_capture_name(recipient_number, text_content):
    """Se a msg do admin tem nome próprio, atualiza customer_name no Mongo."""
    if conversation_collection is None:
        return
    try:
        nome_extraido = extrair_nome_da_msg_admin(text_content)
        if not nome_extraido:
            return
        doc_atual = conversation_collection.find_one({'_id': recipient_number}, {'customer_name': 1})
        nome_atual = doc_atual.get('customer_name') if doc_atual else None
        if nome_extraido != nome_atual:
            conversation_collection.update_one(
                {'_id': recipient_number},
                {'$set': {'customer_name': nome_extraido, 'name_transition_stage': 1}},
                upsert=True
            )
            print(f"🪄 [Auto-nome via admin] {recipient_number} → '{nome_extraido}' (anterior: {nome_atual or 'nenhum'})")
    except Exception as e:
        print(f"⚠️ Erro auto-nome: {e}")
...
# IDs das mensagens que o próprio código enviou via send_whatsapp_message.
# O webhook usa esse set para distinguir "eco do bot" de "admin digitou no celular".
bot_sent_ids = set()
BOT_SENT_IDS_MAX = 500

def register_bot_msg(msg_id):
    if not msg_id:
        return
    bot_sent_ids.add(msg_id)
    if len(bot_sent_ids) > BOT_SENT_IDS_MAX:
        bot_sent_ids.clear()
...
def receive_webhook():
    ...
    try:
        ...
        # Mensagem fromMe: pode ser eco do código OU admin digitando manualmente no celular do bot
        if key_info.get('fromMe'):
            msg_id_check = key_info.get('id')
            if msg_id_check and msg_id_check in bot_sent_ids:
                # Foi o próprio código que mandou → ignora para não duplicar no histórico
                bot_sent_ids.discard(msg_id_check)
                return jsonify({"status": "ignored_bot_echo"}), 200
            # Admin digitou manualmente no celular do bot → salva no histórico do cliente
            ...
            handle_admin_manual_message(message_data)
            return jsonify({"status": "admin_manual_saved"}), 200
...
def handle_admin_manual_message(message_data):
    """
    Salva no histórico do cliente mensagens que o admin digitou MANUALMENTE no celular do bot.
    NÃO dispara IA — só entra como contexto para quando o cliente responder.
    """
    try:
        key_info = message_data.get('key', {})
        remote_jid = key_info.get('remoteJid', '')
        ...
        if message.get('conversation'):
            ...
            try:
                audio_data = base64.b64decode(message['base64'])
                os.makedirs("/tmp", exist_ok=True)
                tmp_path = f"/tmp/admin_audio_{recipient_number}.ogg"
                with open(tmp_path, 'wb') as f:
                    f.write(audio_data)
                transcrito = transcrever_audio_gemini(tmp_path, contact_id=recipient_number)
                try:
                    os.remove(tmp_path)
                except Exception:
                    pass
                if transcrito and not transcrito.startswith("["):
                    text_content = transcrito
            except Exception as e:
                print(f"⚠️ Erro ao transcrever áudio manual do admin: {e}")

        if not text_content:
            return

        append_message_to_db(recipient_number, 'assistant', text_content)
        print(f"📤 [Admin manual] Mensagem salva no histórico de {recipient_number}: '{text_content[:60]}...'")

        try_auto_capture_name(recipient_number, text_content)

    except Exception as e:
        print(f"❌ Erro em handle_admin_manual_message: {e}")
...
if modelo_ia is not None and conversation_collection is not None and agenda_instance is not None:
    ...
    # Injeta o callback para rastrear ids das mensagens enviadas pelo bot (dedup fromMe)
    evolution_api._sent_ids_callback = register_bot_msg
    print("✅ [Boot] Callback de rastreamento de msg_id registrado no evolution_api.")

    print("⚡️ [Boot] Registrando webhook na Evolution API...")
    try:
        evolution_api.configurar_webhook()
    except Exception as e:
        print(f"⚠️ Erro ao registrar webhook no boot: {e}")
